...
def save_lead(telegram_id, username, name, answers, lead_magnet):
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT INTO leads (telegram_id, username, name, answers, lead_magnet) VALUES (?, ?, ?, ?, ?)",
...
def get_stats():
    ...
    by_magnet = conn.execute(
        "SELECT lead_magnet, COUNT(*) as cnt FROM starts GROUP BY lead_magnet ORDER BY cnt DESC"
    ).fetchall()
    by_magnet_leads = conn.execute(
        "SELECT lead_magnet, COUNT(*) as cnt FROM leads GROUP BY lead_magnet ORDER BY cnt DESC"
    ).fetchall()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Video yuborish (ixtiyoriy — fayl bo'lmasa o'tkazib yuboradi)
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "/stats — Statistika (start, lead, konversiya)\n"
        "/leads — So'nggi 20 lead\n"
        "/usernames — Telegram username'lar\n"
        "/export — Barcha leadlar CSV sifatida\n\n"
        "━━━━━━━━━━━━━━━━━━\n"
...
def main():
    ...
    if port:
        ...
        if primary:
            webhook_url = f"https://{primary}/webhook"
            logger.info("Webhook rejim: %s", webhook_url)
            app.run_webhook(
                listen="0.0.0.0",
                port=port,
                url_path="/webhook",
            ...
            logger.warning("REPLIT_DOMAINS topilmadi, polling rejimga o'tildi")
